...
public:
    ...
    CPUState(std::array<char, 512> instructions) : programCounter(0), instructions(instructions), callStackTop(-1) {}
    ...
    void displayState() const {
        ...
        std::cout << "Program Counter: " << static_cast<int>(programCounter) << std::endl;
        std::cout << "Carry Flag: " << (carryFlag ? "1" : "0") << std::endl;
        std::cout << "Zero Flag: " << (zeroFlag ? "1" : "0") << std::endl;

        std::cout << "Registers:" << std::endl;
        for (int i = 0; i < 8; ++i) {
            std::cout << "R" << i << ": " << static_cast<int>(registers[i]) << std::endl;
        }
...
void execute(unsigned short instruction, CPUState &cpu)
{
    ...
    char res;
    bool jump;
    std::cout << opcode << "op " << operand_1 << ":" << operand_2 << ": " << operand_3 << ": " << immediate << std::endl;
    switch (opcode)
    {
    case NOP:
        // No operation
